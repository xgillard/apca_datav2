...
use crate::{
  data::{
    ...
    SingleTrade, SnapshotData, TimeFrame, TradeData,
  },
  errors::{maybe_convert_to_hist_error, Error},
};
...
impl Client {
  ...
  pub async fn snapshots_multi(
    ...
    let rsp = self
      ...
      .await
      .map_err(maybe_convert_to_hist_error)?
      .json::<HashMap<String, SnapshotData>>()
      .await?;
    Ok(rsp)
  }
  /// The Snapshot API for multiple tickers provides the latest trade,
...
impl<'a> FetchNextPage<'a, MultiTrades> for FetchNextTrades<'a> {
  fn fetch(
    self: Pin<&Self>,
    token: Option<String>,
  ) -> Pin<Box<dyn Future<Output = Result<MultiTrades, Error>> + 'a>> {
    Box::pin(
      self
        .client
        .trades_paged(self.symbol, self.start, self.end, self.limit, token),
    )
  }
}
...
impl<'a> FetchNextPage<'a, MultiQuotes> for FetchNextQuotes<'a> {
  fn fetch(
    self: Pin<&Self>,
    token: Option<String>,
  ) -> Pin<Box<dyn Future<Output = Result<MultiQuotes, Error>> + 'a>> {
    Box::pin(
      self
        .client
        .quotes_paged(self.symbol, self.start, self.end, self.limit, token),
    )
  }
}
...
impl<'a> FetchNextPage<'a, MultiBars> for FetchNextBars<'a> {
  fn fetch(
    self: Pin<&Self>,
    token: Option<String>,
  ) -> Pin<Box<dyn Future<Output = Result<MultiBars, Error>> + 'a>> {
    Box::pin(self.client.bars_paged(
      self.symbol,
      self.start,
      self.end,
      self.timeframe,
