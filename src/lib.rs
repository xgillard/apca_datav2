...
use data::{
  AuthData, BarData, MultiBars, MultiQuotes, MultiTrades, QuoteData, SingleQuote, SingleSnapshot,
  SingleTrade, SnapshotData, Source, TimeFrame, TradeData,
};
use errors::Error;
use futures::Stream;
...
impl Apca {
  ...
  pub async fn snapshots_multi(
    &self,
    symbols: &str,
  ) -> Result<HashMap<String, SnapshotData>, Error> {
    self.hist.snapshots_multi(symbols).await
  }
  /// The Snapshot API for multiple tickers provides the latest trade,
  /// latest quote, minute bar daily bar and previous daily bar data for
  /// the given ticker symbols.
  pub async fn snapshots_multi_vec(
    &self,
    symbols: &[&str],
  ) -> Result<HashMap<String, SnapshotData>, Error> {
  ...
  ) -> Result<MultiTrades, Error> {
    self
      .hist
      .trades_paged(symbol, start, end, limit, page_token)
      .await
  }
  /// This endpoint returns quote (NBBO) historical data for the requested security.
  pub async fn quotes_paged(
    &self,
    symbol: &str,
    start: DateTime<Utc>,
    end: DateTime<Utc>,
    limit: Option<usize>,
    page_token: Option<String>,
  ) -> Result<MultiQuotes, Error> {
    self
      .hist
      .quotes_paged(symbol, start, end, limit, page_token)
      .await
  }
  /// This endpoint returns aggregate historical data for the requested security.
  pub async fn bars_paged(
    &self,
    symbol: &str,
    start: DateTime<Utc>,
    end: DateTime<Utc>,
    ...
    limit: Option<usize>,
    page_token: Option<String>,
  ) -> Result<MultiBars, Error> {
    self
      .hist
      .bars_paged(symbol, start, end, timeframe, limit, page_token)
      .await
  }
